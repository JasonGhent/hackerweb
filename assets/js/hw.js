...
console.log(sessionStorage);
  ...
  var isSafari = function(){
    ...
    if (ua.indexOf('safari')!=-1){ 
      if(ua.indexOf('chrome') >-1) return false; // chrome
      else return true; // saf
    }
    else return false;
  };
  ...
  if(isSafari()){
    viewPortTag.content += ", minimal-ui";
  }
    ...
    render: function(opts){
      ...
      if (cached){
        ...
        hnapi.news(function(data){
          ...
          amplify.store('hacker-news-cached', true, { expires: 1000*60*60*24 }); // 1 day

          hw.comments.stack.consume(); // begin consumption monitor
          hw.comments.stack.add(data.reverse()); // reverse ordering since stack will be FILO

          loadNews(data); // render the fetched data
    ...
    stack: {
      ...
      consume: function(){
        ...
        setInterval(function(){
          // process stack items at a concurrency of 4
      ...
      add:function(data){
        var self = this;
        for(var i=data.length-1;i>=0;i--){ //all
    ...
    render: function(id){
      ...
      if (post){
        ...
        if (post){
          ...
          // load immediately as this is a result of direct URL call for item
          this.getItemDetails(id,true);
        }
        this.getItemDetails(id,true);
      }
    },
    getItemDetails:function(id,post,cb){
      var self = this;

      //@TODO determine how important this is and reimplement if necessary.
      var showError = function(){
        if (post){
          delete post.loading;
          post.load_error = true;
//          self.loadPost(post, id);
//        } else {
//          self.loadPost({load_error: true}, id);
        }
//        hw.pub('logAPIError', 'comments');
      };
        
      hnapi.item(id, function(data){
//        @TODO determine if this is an issue and fix when reimplementing the queue
//        // Avoiding the case where the wrong post is loaded when connection is slow
//        if (hw.comments.currentID != id) return;
//        if (!data || data.error && hw.currentView == 'comments') showError();
        ...
        if(post){
          // post param is callback
          if(post!==true) post();
          // load it.
          else self.loadPost(data, id);
        }
      }, function(e){
        if (hw.comments.currentID != id) return; // showError();
      });
    },
