(function(w){
  ...
  document.getElementsByTagName('head')[0].appendChild(viewPortTag);
			...
			if (cached){
				...
				hnapi.news(function(data){
					...
					amplify.store('hacker-news-cached', true, {
						expires: 1000*60*45 // 45 minutes
					});
					amplify.store('hacker-news2', null);
          hw.comments.cacheQueue(data);
					loadNews(data);
	...
	hw.comments = {
		currentID: null,
    cacheQueue:function(data){
      //@TODO: make this a true queue.
      var self = this;
      for(var i=0;i<data.length;i++){ //all
        var item = data[i];
        (function(item){
          setTimeout(function(){
            self.getItemDetails(item.id);
          },500);
        })(item);
      }
    },
    ...
    getItemDetails:function(id,post){
      var self = this;
